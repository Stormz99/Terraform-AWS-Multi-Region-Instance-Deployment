variable "region" {
  type = string
  description = "This specifies the region where the resource is created and it is filled by the *.tfvars file."
}

variable "availability_zones" {
  type = list(string)
  description = "A list of at least, two availability zones in the region."
}
...
variable "public_key" {
  ...
  # just make sure the provate key is renamed to my_key_${var.region}.pem, you'd have two e.g. my_key_eu-west-1.pem and my_key_eu-central-1.pem
...
variable "environment" {
  type = string
  description = "The environment in which this is created"
}
